...
- Operational skills built: `session-memory` and `agent-finder` (subagent selection/delegation, engine `scripts/agents.py`). **Single-sourced:** canonical copies live in `example-project/.claude/`; the factory `.claude/` holds symlinks (incl. `build-hooks.py` + shared hook fragments) — edit in `example-project/`, never copy.
