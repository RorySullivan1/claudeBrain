...
- Operational skills built: `session-memory`, `agent-finder`, and `knowledge-router` (routes durable knowledge; owns the `.claude/context/` reference-notes tier — `INDEX.md` + `notes/*.md` via `scripts/context.py`, surfaced by a SessionStart hook). **Single-sourced:** canonical copies live in `example-project/.claude/`; the factory `.claude/` holds symlinks (incl. `build-hooks.py` + shared hook fragments) — edit in `example-project/`, never copy.
