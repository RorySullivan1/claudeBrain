...
| Print HTML — static rules | `skills/weasyprint-print-html/scripts/lint_print_html.py` | `agent-runnable` | Pure source analysis; `probes/flawed.html` is the negative control | 2026-09-15 — flawed 7E/8W, both positive controls clean |
| Print HTML — PDF render | `skills/weasyprint-print-html/scripts/render.py` | `agent-runnable` | WeasyPrint installs from PyPI wheels with no system libs; **exit 1 = rendered WITH warnings**, not success | 2026-09-15 — 2-page render, font + strict-fetch warnings caught |
| Print HTML — CSS support table | `skills/weasyprint-print-html/probes/probe_css_support.py` | `agent-runnable` | The table IS the linter's rule set, so it is drift-checked against the installed WeasyPrint | 2026-09-15 — 2 controls PASS, 33 rows, no drift |
| Print HTML — page appearance | `skills/weasyprint-print-html/scripts/snapshot.py` | `human-gated` | Needs poppler (`pdftoppm`/`pdffonts`), a system binary that is not pip-installable. No page image has been looked at | — |
| Factsheet data contract | `skills/factsheet-template/scripts/validate_context.py` | `agent-runnable` | Fail-closed gate; discrimination proved on six injected defects | 2026-09-15 — valid sample passes, all six caught |
