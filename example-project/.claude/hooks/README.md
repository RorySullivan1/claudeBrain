...
It reads `.claude/prose-budget.json`, and **that file is the adoption marker**: absent, every entry point returns nothing, so a project that vendors this and adopts nothing sees no output. Keys are `module`, `class`, `function`, `attribute` and `comment_run` (line caps), `include` (roots to scan), and `baseline` (a path to a location-to-reason mapping whose entries are exempt). Baseline keys are qualified names rather than line numbers — a comment run, which has no name, is keyed by a short content hash of its own text — so an edit above a docstring does not invalidate an entry and fail the gate for an unrelated reason; a comment-run key churns only when that comment is rewritten.
