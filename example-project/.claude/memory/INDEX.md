...
- Skill families cover VSTO, **VBA** (full lifecycle + tests), Python, quant, docs, GitHub.
- **Developer agents** pair with the families: `python-developer`, `vsto-developer`,
  `finance-quantitative-developer`. Factory now has **7 meta-skills** (skill/agent×4/workflow/
  context-vs-skill) + the `add-context` scaffolder.
- **Roadmap → orchestration** layer drives plan-based iterative dev (`development-mapping`,
  ...
  `.meta/version` + ship-version spine.
- Memory via `session-memory`. 5 PRs open (#22–#26), none merged.
...
- [2026-06-27] Orchestration = a workflow, not an agent; the planner = a skill+command. Roadmap is the route, `.meta/version` the cursor; the orchestrator reuses ship-version — sessions/2026-06-27-2138-github-and-roadmap-assets.md
- [2026-06-27] Build the assessed enhancements via one-file-per-agent fan-out; integrate shared files (catalogs/READMEs) sequentially. Context briefs point to skills (skills own how-to) — sessions/2026-06-27-2209-code-review-and-enhancements.md

## Threads          (open items; remove when closed)
- Merge the PR stack #22–#26 (refs between #25/#26 are softened/conditional → flexible order).
