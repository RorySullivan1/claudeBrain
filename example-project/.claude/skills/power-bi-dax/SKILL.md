...
`ALL ( 'Date'[Date] )` removes the filter on that *one* column only, so on a matrix whose axis
is `'Date'[Month]` or `'Date'[Year]` the period filter survives and the running total collapses
to the period value. Capturing `LastVisibleDate` in a variable *before* the `CALCULATE` is
essential — inside the `CALCULATE` the date context is gone. For a within-year reset, use
