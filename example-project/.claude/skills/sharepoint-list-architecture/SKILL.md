...
- **There are no real joins.** Lookup columns *look* relational but each one is a
  costly join (lookup, Person/Group, and Managed Metadata columns all count). The
  "List View Lookup Threshold" is nominally **12 per query** — but the same
  boundaries doc also says a query using **more than 8 joins is blocked** (12 is
  what a *maximal* view returns), so treat **8 as the safe design ceiling** and
  12 as the absolute one. There is no foreign-key engine enforcing integrity for
  you.
...
| A value that lives in and is maintained by another list | **Lookup** | Choice | Costs **1 join** (8-join safe ceiling — see above). Delegation is limited — filtering on lookup subfields is restricted. Use only for genuine shared reference data. |
...
but each lookup spends join budget — cap at 8 per query (see above). **Never
...
- **The 8-join wall.** Lookup, Person/Group, and Managed Metadata columns each cost
  a join; the threshold is nominally 12 per query, but the boundaries doc blocks
  queries above **8** joins, so design to 8. A child list with several lookups plus a
