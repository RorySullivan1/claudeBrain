...
for an authored change. Project state is named by **role** — the enum catalogue, the validator
allow-list, the paste log — with each role's concrete path declared in the consuming project's
`CLAUDE.md` (per `powerapp-canvas-project-management`: four roles, not four filenames). The
`e.g.` paths below are one project's layout, not requirements.
...
1. **Check the catalogue first.** Look the token up in `powerapp-canvas-controls`, then in
   the project's enum catalogue (e.g. `tools/studio-enums.json`) and the validator's
   allow-list. Already grounded → **stop, done**;
...
4. **Record it in all four places, once.** The propagation is the point of the workflow:
   ...
   - the enum catalogue (e.g. `tools/studio-enums.json`) — the token, its properties, its
     OUTPUT property, the Studio defaults seen in the sample, and its provenance;
   ...
   - the paste log (the crossing) and, via **`session-memory`**, the decision if it corrects a
     previous belief.

5. **Verify the propagation.** Run the project's paste validator (e.g.
   `python tools/validate_pa_yaml.py`) — it audits the copies against each other and flags any
   allow-listed token missing from the enums file or the skill. A warning there means step 4 is
   unfinished. → hand-off: a clean cross-check.
...
- **Missing state roles:** the project's `CLAUDE.md` declares no enum catalogue / validator /
  paste log → **stop and set them up first** (route to `powerapp-canvas-project-management`);
  steps 4–5 cannot gate anything against files that don't exist.
...
- State (by role; paths per the project's `CLAUDE.md`): the enum catalogue (e.g.
  `tools/studio-enums.json`), the validator allow-list, the paste log (e.g. `paste-log.md`),
  `.claude/memory/INDEX.md`.
- Tools: the paste validator (e.g. `python tools/validate_pa_yaml.py`) — cross-checks the
  catalogue copies.
