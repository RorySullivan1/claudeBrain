...
order is the value — from step 3 onward **each step's output is the next step's input** (steps
1 and 2 read disjoint inputs and may run in parallel), and skipping the grounding steps is how
you discover a wrong column name *after* the paste.

Project state is named by **role** — the golden-source schema file, the paste validator, the
build book, the paste log — with concrete paths declared in the consuming project's
`CLAUDE.md` (per `powerapp-canvas-project-management`); the `e.g.` paths below are one
project's layout.

**Inputs:** a screen to build or rebuild and the behaviour it must support.
**Output:** authored, geometry-checked, validated, PASTE-audited screen source in the
authored-source directory, plus a hand-off note; a paste-log row once the human reports the
outcome.

## Steps

1. **Ground the data (schema first; parallel with step 2).** Every column the screen reads or
   writes must resolve to a `name:` in the golden-source schema file (e.g.
   `schema/schema.yaml`). Missing column → change the golden source *first* via
...
2. **Ground the controls (parallel with step 1).** List the control types the screen needs and
   check each against **`powerapp-canvas-controls`** / the enum catalogue (e.g.
   `tools/studio-enums.json`). Anything ungrounded → run the
...
4. **Author.** Column tokens from step 1, control tokens from step 2. Apply
   **`powerapp-canvas-development`** (file structure) and **`power-fx-development`**
   ...
   multi-line and `: `-bearing formulas as block scalars. → hand-off: authored source files.
...
6. **Validate.** Run the project's paste validator (e.g. `python tools/validate_pa_yaml.py`) —
   schema, tokens, icons, `IfError` typing, gallery `OnSelect`, cross-file component
   contracts, positioning-off-another-control. A new failure class earns a new lint **in the
   same change**. → hand-off: a green validator plus its output.

7. **Audit.** Spawn **`../agents/pre-paste-review.md`** for a **PASTE / DO-NOT-PASTE**
   verdict, passing step 6's validator output so the agent doesn't re-run it — scope the agent
   to what the validator can't judge: schema resolution against the snapshot, delegation
   semantics, and paste shape. → hand-off: the verdict.

8. **Hand off (human gate).** Per **`studio-transfer`**: update the build book (e.g.
   `BUILD-BOOK.md`) with anything
...
- **Ordering is a precondition from step 3 onward.** Steps 1 and 2 read disjoint inputs
  (schema vs enum catalogue) and may run in parallel; from step 3 on, don't start step N until
  step N−1's hand-off exists — a control grounded after authoring is a rebuild, not a fix.
- **Branch (step 1):** a column is missing → amend the golden-source schema file and flag
  re-provisioning before any authoring; else → step 3 once step 2 also completes.
...
- **Loop (validate/audit):** step 6 red or step 7 **DO-NOT-PASTE** → back to step 4 with the
  findings; re-run 5–7 (the validator runs once per iteration, in step 6 — the audit consumes
  its output rather than re-running it). Terminal state: validator green **and** a PASTE
  verdict. Two passes with no progress → **stop and report** the blocker.
- **Missing state roles:** the project's `CLAUDE.md` declares no schema file, validator, build
  book, or paste log → **stop and set them up first** (route to
  `powerapp-canvas-project-management`); steps 1, 6, 8 and 9 gate on them.
