<!--
  This is the example-project showcase from the claudeBrain factory.
  It plays the part of a real downstream repo so you can see a populated .claude/
  in context. Copy this file (and the .claude/ next to it) into your own project
  and replace the contents with your real specifics. Keep CLAUDE.md short — it
  loads on EVERY session, so it should hold only what's true every time, and point
  into .claude/context/ for detail.
-->

# example-project

A demonstration project that ships an Office productivity tool: a **VSTO Excel
add-in** (C#) backed by a set of **Python** data and packaging scripts, with
end-user and developer **documentation**. It exists to show what a real consumer
of the `claudeBrain` factory looks like once the assets are in place.

## Architecture
- **Add-in** (`src/AddIn/`): VSTO Excel add-in, C# — Ribbon UI + task panes.
- **Tooling** (`tools/`): Python scripts for data prep and build/packaging.
- **Docs** (`docs/`): developer docs (contributors) and user guides (the desk).

## Constraints
- Corporate-managed Windows; no admin rights on end-user machines.
- Office is Click-to-Run (Microsoft 365); add-in deploys via ClickOnce.
- Python tooling targets the pinned interpreter in `tools/.python-version`.

## Capabilities
Task-scoped **skills** (`.claude/skills/`) and isolated **agents** (`.claude/agents/`) both
auto-load every session by their `description:` — Claude picks the right one; you don't invoke
them by hand (agents also via `@agent-<name>`). Coverage spans: the **VSTO/.NET add-in** and its
**Python tooling** (write / review / maintain / deploy), **quant** analysis, **docs**, a
**branding → presentation** pipeline (an objective flow architect, then design, then
format-specific builders), and a **Power Platform** family — the canvas app itself (controls,
pa-yaml authoring, layout, project discipline, in-app SVG charting, editable grids), the Power Fx
inside it, the SharePoint backend behind it, and `studio-transfer` for projects whose only route
into Power Apps Studio is a human with a clipboard — plus cross-cutting **knowledge, memory, and
token-economy** helpers.

For the full always-current inventory — every skill, agent, command, and workflow with a
one-line purpose — read **`.claude/CATALOG.md`** (generated by `.claude/hooks/catalog.py`;
regenerate with `/reindex`). Don't enumerate assets here: the catalog is the single source, and
the harness already surfaces each skill/agent by its `description:`.

## Reference Docs
See `.claude/context/README.md` for two tiers of reference: whole-stack operating briefs
and on-demand reference notes.
- VSTO specialist brief — `vsto-project-instructions.md`
- Python full-lifecycle brief — `python-project-instructions.md`
- C/C# brief — `c-csharp-project-instructions.md`
- (also: C++ and VBA briefs)
- One-way transfer model (the clipboard air gap) — `air-gap.md`; its *how-to* counterpart is
  the `studio-transfer` skill, which is the worked brief↔skill split `context-vs-skill` teaches
- **Reference notes** — small declarative cards in `context/notes/`, catalogued in the
  always-loaded `context/INDEX.md` (managed by `knowledge-router`).

## Memory & decisions
State and decisions carry across sessions via the `session-memory` skill:
`.claude/memory/INDEX.md` (auto-loaded) plus append-only `sessions/*.md` logs.

## Roadmap & versioning
Two altitudes of prospective state under `.meta/`:
- **Roadmap** (`.meta/roadmap/`) — the whole-project plan: an `INDEX.md` dashboard plus
  `stages/` of version cards (objective → stages → versions → milestones). Authored with
  `/roadmap-set` (the `development-mapping` skill supplies the thinking) and surfaced each
  session by the `roadmap_status` hook. It's the route.
- **Version** (`.meta/version`) — the single unit of work currently in flight (the cursor):
  a semver label + its goals. `/version-set` records it; `/version-ship` (the `ship-version`
  workflow) derives the branch + PR title/body from its goals.

The `advance-roadmap-step` workflow connects them: it graduates the cursor card into
`.meta/version`, drives implement → review → reiterate → `goal-auditor` assessment, stops
for approval, then ships. The `roadmap_guard` hook warns at push time when the two drift
(a version shipped but still `planned`/`in-progress` on the map). Both files are prospective
and complement memory's retrospective log.

## Conventions
- Branch per change; conventional-commit-style messages.
- Python formatted with the project's configured formatter/linter before commit.
- Skill folder name always equals the skill's `name:` frontmatter.

## Compact Instructions
On compaction, preserve: the ClickOnce deployment constraint, the no-admin-rights
limit, the pinned Python version, and which skills cover VSTO vs. Python vs. docs.
